...
const { getModels } = require("./model-fetcher");
    ...
    steps: [
      {
        ...
        form: async (context) => {
          ...
          if (config.backend === "Local llama.cpp") {
            ...
            models = await getModels('OpenRouter', config.api_key);
          } else if (config.backend === "Mistral") {
            models = await getModels('Mistral', config.api_key);
          }
...
const modelpatterns = (config) => ({
  LargeLanguageModel: {
    ...
    hyperparameter_fields: ({ table, configuration }) => [
      ...
      ...(config.backend === "OpenRouter" || config.backend === "Mistral"
        ? [
            {
              name: "max_tokens",
              label: "Max tokens",
            ...
            },
            {
              ...
              label: "Top P",
              type: "Float",
              attributes: { min: 0, max: 1, decimal_places: 2 },
              default: 1,
            },
          ]
        : []),
    ...
    predict: async ({
      ...
      for (const row of rows) {
        ...
        const output = await getCompletion(mdlConfig, { ...opts, prompt });
